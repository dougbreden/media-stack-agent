...
function Wait-ContainerHealthy {
    ...
    while ($elapsed -lt $TimeoutSec) {
        ...
        # Containers without a healthcheck return "": treat as up when running
...
# Stop qBittorrent first so the lockfile is not held open when we delete it.
docker compose -f $ComposeFile stop qbittorrent 2>&1 | Out-Null

# Clean stale qBittorrent lockfile - left behind when the container is killed
# while writing. Causes qBittorrent to hang in D state on next start.
...
# Force-recreate gluetun to reset iptables rules from scratch.
...
# Verify VPN tunnel is actually connected (not just container-healthy)
...
# Start qBittorrent against the fresh gluetun network namespace
...
if (Wait-ContainerHealthy "qbittorrent" 90) {
    ...
    # Spot-check tracker connectivity via qBittorrent API
    try {
...
$allContainers = $psRaw | ForEach-Object {
