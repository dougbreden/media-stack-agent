...
|---|---|---|
| **Homarr** (dashboard) | http://<server-ip>:7575 | Tiles linking to all services |
| **Jellyfin** (watch) | http://<server-ip>:8096 | Stream movies and TV |
| **Jellyseerr** (request) | http://<server-ip>:5055 | Request new movies and TV shows |

Via Tailscale (from anywhere, e.g. cellular): replace `<server-ip>` with `<tailscale-ip>`.
...
1. Open **Jellyseerr** → http://<server-ip>:5055
...
**Desktop (browser):** http://<server-ip>:8096 — works in any browser, direct play for most files.

**iOS — Jellyfin app (free):**
- Add server: `http://<server-ip>:8096` (LAN) or `http://<tailscale-ip>:8096` (cellular via Tailscale)
...
- If nothing plays: the stack may be down — check http://<server-ip>:7575 (Homarr will show offline tiles)
